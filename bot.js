...
function respond() {
  var request = JSON.parse(this.req.chunks[0]),
      botRegex = /^\/MHH Bot/;  botRegexDL = /^\/rst/i;botRegexSalt = /^\/salt/;botRegexYub = /^\/yubnub/
      botRegexAd=/^\/advance/;botRegexGTA = /^\/gta/; botRegexSC = /^\/sch/i; botODB = /(.*\s+)(.*odb)(\s+.*)/i; botDuck = /^\/duck/;
      botRegexP = /^\/ply/i;  botRegexTw = /^\/twitch/i; botRegexSb = /^\/sub/; botRegexSh = /^\/shrug/; botRegexWk = /^\/users/;
      botRegexRu = /^\/rules/; botRegexYu = /^\/youtube/i; botRegexSd = /^\/schedule/; botRegexTr = /^\/trades/;
  ...
  if(request.text && botRegex.test(request.text)) {
    ...
    this.res.writeHead(200);
    postMessage("http://daddyleagues.com/mhh/team/"+request.text.substring(5,8)+"/depthchart");
    this.res.end();
  } 
    ...
    var rep = req.replace(/ /,"+");
    postMessage("http://daddyleagues.com/mhh/players?name="+rep+"&position=all&team=all");
    this.res.end();
  }  
    ...
    this.res.writeHead(200);
    postMessage("http://daddyleagues.com/mhh/schedules");
    this.res.end();
  } 
  else if(request.text && botRegexTr.test(request.text)) {
    this.res.writeHead(200);
    postMessage("https://www.daddyleagues.com/mhh/forum/forum/5074");
    this.res.end();
  }
  else {
